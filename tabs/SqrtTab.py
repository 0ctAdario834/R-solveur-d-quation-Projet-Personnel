...
def create_sqrt_tab(notebook, window):
    ...
    notebook.add(tab_sqrteqnslvr,text=" | Équation Racine Carrée | ")
    tab_sqrteqnslvr.config(background='#999999')

    entry_accuracy = None

    name = Label(tab_sqrteqnslvr,
                 text="Résolveur d'Équations Racine Carrée",
    ...
    def SolveSqrtEquation():
        ...
        try:
            if a == 0 or b == 0:
                messagebox.showerror("Erreur", "Ceci n'est pas une équation de racine carrée.")
                return
